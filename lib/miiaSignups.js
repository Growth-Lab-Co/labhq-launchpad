...
import { getIntakeProgress } from "./intakeDrafts.js";
...
// Full-scan lookup, same shape as getSignupByTenantSlug's own fallback -
// fine at current signup volume. Backs the subscription.updated webhook
// backstop (app/api/miia/webhook/route.js), which only has a Stripe
// customer id to work from.
export async function getSignupByStripeCustomerId(stripeCustomerId) {
  if (!stripeCustomerId) return null;
  const { blobs } = await signupsStore().list();
  const records = await Promise.all(blobs.map(({ key }) => getSignupById(key)));
  return records.find((r) => r?.stripeCustomerId === stripeCustomerId) || null;
}
...
// Derives intake/deploy status live from lib/deployments.js (and, for an
// interview that hasn't finished yet, lib/intakeDrafts.js) rather than
// trusting a stored copy - see file header. intakeStatus is "complete" once
// a deployment record exists, "in_progress" (with a turn count) once a
// draft exists but no deployment yet, otherwise "not_started" - so ops can
// see a genuinely stalled customer instead of guessing from silence.
async function withLiveStatus(record) {
  if (!record.tenantSlug) {
    return { ...record, intakeStatus: "not_started", deployStatus: "not_started", intakeTurnCount: 0 };
  }
  ...
  if (latest) {
    ...
    const progress = await getIntakeProgress(record.tenantSlug).catch(() => ({ status: "not_started", turnCount: 0 }));
    intakeStatus = progress.status;
    intakeTurnCount = progress.turnCount;
  }
