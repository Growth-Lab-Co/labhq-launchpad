...
// Cliniko's complete, documented shard list (docs.api.cliniko.com/guides/urls,
// read 2026-07-28) - an allowlist, not a loose pattern match, so a garbage
// or malformed key can never build a bogus hostname (found during testing:
// a fake key ending in digits that merely LOOKED shard-shaped produced a
// dead host and a confusing "fetch failed" instead of an honest "invalid
// key" response).
const KNOWN_SHARDS = new Set(["au1", "au2", "au3", "au4", "au5", "ca1", "uk1", "uk2", "uk3", "eu1"]);

function baseUrlFor(apiKey) {
  const match = SHARD_RE.exec((apiKey || "").trim());
  const candidate = match ? match[1].toLowerCase() : "";
  const shard = KNOWN_SHARDS.has(candidate) ? candidate : "au1";
...
async function clinikoFetch(apiKey, path, { method = "GET", body } = {}) {
  ...
  try {
    ...
    throw new Error(`Couldn't reach Cliniko - ${e.cause?.message || e.message}`);
  }
