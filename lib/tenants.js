...
import { saveWidgetKeyMapping } from "./widgetKeys.js";

const STORE_NAME = "tenants";

// Public key for the in-house widget (job 1, 2026-07-29 build) - safe to
// expose client-side, same trust model as a Stripe publishable key (see
// lib/widgetKeys.js). Miia tenants only; agency tenants keep the GHL-hosted
// widget and have no use for one.
function generateWidgetKey() {
  return `mia_pk_${randomBytes(16).toString("hex")}`;
}

// Lazily backfills a widgetKey for any Miia tenant that predates this field
// (every tenant created before tonight) - called wherever the widget
// snippet/key is actually needed, so there's no separate migration script
// to run and no tenant is ever silently left without one.
export async function ensureWidgetKey(tenant) {
  if (!tenant || tenantProductType(tenant) !== "miia") return tenant;
  if (tenant.widgetKey) return tenant;
  const widgetKey = generateWidgetKey();
  await saveWidgetKeyMapping(widgetKey, tenant.slug);
  const updated = await updateTenant(tenant.slug, { widgetKey });
...
export async function createTenant({
  ...
  // Widget key generated up front for new Miia tenants only - agency
  // tenants keep the GHL-hosted widget and have no use for one.
  const widgetKey = productType === "miia" ? generateWidgetKey() : null;
  const record = {
    slug: key,
    name,
    assistantName,
    ...
    plan: plan || null,
    // First-touch UTM attribution (see lib/utm.js), threaded from checkout
    // metadata through the signup record - only ever set for product:"miia"
    // tenants, same as plan above.
  ...
  if (widgetKey) await saveWidgetKeyMapping(widgetKey, key);
  return record;
}
