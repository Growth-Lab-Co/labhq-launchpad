...
import { askClaude, askClaudeStructured, streamClaude, extractJson } from "./claude.js";
...
import { listAvailableWidgetActions } from "./widgetActions.js";
...
export function buildSystemPrompt(deployment, tenant, bookingContext) {
...
- Pricing: ${cv.pricing_summary || "not specified - if asked, say pricing isn't loaded yet and offer to have the team follow up, never guess a number"}
...
Reply rules:
- 2 to 4 sentences for a normal question. Never a wall of text. If the honest answer genuinely needs more than that, give the short version and offer to go deeper rather than dumping it all at once.
- Answer the question actually asked, not everything you know about the topic. If someone asks the price of one thing, give that price, don't recite the whole pricing table.
- Always end with a next step or a question, never a full stop on a list of facts.
- Plain, warm, direct. No preamble, no "great question" or "good question", no restating the question back before answering it.
- No em dashes (—) anywhere in your reply. Use a comma, a full stop, or a simple hyphen (-) instead.
- No emoji. No exclamation marks.
- Write in Australian English, in the tone described above.
- Keep the reply under ${MAX_REPLY_WORDS} words, as a hard ceiling, not a target.
- Never invent services, prices, or availability that aren't in the business context above.${
    tenant?.assistantName
      ? ` If you refer to yourself or your product by name, it is spelled exactly "${tenant.assistantName}" (character for character) - never substitute a different or more common spelling.`
      : ""
  }`;
}
...
// Shared by both the GHL pipeline (processInboundMessage) and the in-house
// widget (app/api/widget/message/route.js) - a customer can ask to book
// through either channel, and both should file the same honest booking-
// REQUEST ops task when they confirm a real offered time. `tenantRecord` is
// the already-fetched tenant (both callers have one on hand - never
// re-fetched here).
export async function fileBookingRequestIfConfirmed({ tenantSlug, businessName, deploymentId, tenantRecord, messages, inboundText, reply }) {
  const bookingContext = await buildBookingContext(tenantRecord);
  if (!bookingContext) return null;
  const confirmation = await detectBookingConfirmation({ bookingContext, messages, inboundText, reply }).catch(() => null);
...
//
// Returns { reply, actions }, not a bare string - `actions` is always []
// unless offerActions is explicitly true. offerActions is the one and only
// channel-awareness switch for suggested action buttons (job 1, 2026-07-31):
// only netlify/functions/widget-reply-background.mjs (the in-house widget)
// passes true. processInboundMessage below (SMS, Facebook/Instagram, GHL's
// own webchat) never does, so those channels structurally can never render
// a button - there's no client surface there that could render one anyway.
export async function generateReply({ deployment, messages = [], inboundText, offerActions = false }) {
  ...
  const claudeMessages = [...history, { role: "user", content: inboundText }];

  // listAvailableWidgetActions already filters to only kinds with a real,
  // resolved destination for this tenant (lib/widgetActions.js) - if none
  // exist, there's nothing honest to offer, so this collapses to the exact
  // same plain-text path as offerActions: false.
  const availableActions = offerActions ? listAvailableWidgetActions({ tenant, deployment }) : [];
  if (!availableActions.length) {
    const raw = await askClaude({ system, messages: claudeMessages, maxTokens: 400 });
    return { reply: enforceWordLimit(raw, MAX_REPLY_WORDS), actions: [] };
  }

  const actionsPrompt = `\n\nYou may also suggest up to 3 relevant next-step buttons, from this fixed list only - you can never write or invent one of your own:\n${availableActions
    .map((a) => `- ${a.key}: "${a.label}" - offer this when ${a.when}`)
    .join("\n")}\nOnly suggest an action when it's genuinely relevant to what the visitor just asked - most replies need none at all, don't force one in.`;

  const schema = {
    type: "object",
    properties: {
      reply: { type: "string", description: "Your conversational reply, 2 to 4 sentences." },
      suggested_actions: {
        type: "array",
        items: { type: "string", enum: availableActions.map((a) => a.key) },
        maxItems: 3,
        description: "0 to 3 action keys from the fixed list, only when genuinely relevant this turn.",
      },
    },
  ...
  try {
    const result = await askClaudeStructured({
      system: system + actionsPrompt,
      messages: claudeMessages,
      toolName: "submit_reply",
      toolDescription: "Submit your reply and any relevant suggested next-step buttons.",
      schema,
      maxTokens: 400,
    });
    const actions = (result.suggested_actions || [])
      .map((key) => availableActions.find((a) => a.key === key))
      .filter(Boolean)
      .slice(0, 3)
      .map(({ key, label, href }) => ({ key, label, href }));
    return { reply: enforceWordLimit(result.reply || "", MAX_REPLY_WORDS), actions };
  } catch (e) {
    // Additive feature, not essential - a failure here must never block a
    // reply the customer is waiting on. Falls back to the exact same plain
    // path as offerActions: false.
    console.error("[BOT] structured reply with actions failed, falling back to plain text:", e.message);
    const raw = await askClaude({ system, messages: claudeMessages, maxTokens: 400 });
    return { reply: enforceWordLimit(raw, MAX_REPLY_WORDS), actions: [] };
  }
}

// Streaming counterpart for the in-house widget (job 1) - same business
// context/booking-grounding as generateReply, but returns a ReadableStream
// of text chunks instead of waiting for the full reply. Caller (the widget
// message API route) is responsible for buffering the streamed text to
// persist the final message once the stream ends. Takes `tenant` directly
// (the widget route already has it) rather than re-fetching it, since this
// is called far more often per-conversation than the GHL path ever was.
export async function streamReply({ deployment, tenant, messages = [], inboundText }) {
  const bookingContext = await buildBookingContext(tenant);
  const system = buildSystemPrompt(deployment, tenant, bookingContext);
  const history = messages.map((m) => ({
    role: m.direction === "outbound" ? "assistant" : "user",
...
export async function processInboundMessage({
  ...
  try {
    ...
    // offerActions defaults false - GHL channels (SMS, Facebook/Instagram,
    // GHL's own webchat) never get suggested-action buttons, see
    // generateReply's own header comment.
