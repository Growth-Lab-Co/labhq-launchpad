...
async function claimTenantSlug({ businessName }) {
...
export async function provisionTenantForSignup(signup) {
  if (signup.provisioningStatus === "success" && signup.tenantSlug) {
    return signup;
  }

  let tenant;
  try {
    tenant = await claimTenantSlug({ businessName: signup.businessName });
  } catch (e) {
    const failed = await updateSignup(signup.id, {
      provisioningStatus: "failed",
      provisioningError: e.message,
      provisioningAttempts: (signup.provisioningAttempts || 0) + 1,
    });
    await alertOps(failed, e).catch((emailErr) =>
      console.error(`[miiaProvisioning] alertOps itself failed for signup ${signup.id}:`, emailErr.message)
    );
    return failed;
  }
  ...
  if (!updated.welcomeEmailSentAt && updated.email) {
    const intakeLink = `${SITE_URL}/${tenant.slug}`;
    const firstName = (updated.contactName || "").trim().split(/\s+/)[0] || null;
    const { subject, html, text } = buildWelcomeEmail({ firstName, intakeLink });
    const result = await sendEmail({ to: updated.email, subject, html, text });
    if (result.ok) {
      return updateSignup(updated.id, { welcomeEmailSentAt: new Date().toISOString() });
    }
