...
import { sendTransactionalEmail, logEmailFailure } from "./emailFailures.js";
...
async function claimTenantSlug({ businessName, healthcareMode, plan, utmSource, utmMedium, utmCampaign }) {
...
async function alertOps(signup, error) {
  ...
  const result = await sendTransactionalEmail({
    context: "provisioning-failed-alert",
    to,
    subject,
    html,
    text,
...
export async function provisionTenantForSignup(signup) {
  ...
  // Wrapped separately from tenant creation above: a bug here (or a Blobs
  // hiccup) must never look like "the whole checkout failed" to the
  // customer - the tenant already exists at this point. Found 2026-07-28:
  // an unhandled throw in this block was silently swallowed by the
  // webhook/status route's own catch-and-console.error, so no welcome
  // email ever went out and nothing recorded why - see
  // lib/emailFailures.js. Logged loudly now instead of thrown.
  if (!updated.welcomeEmailSentAt && updated.email) {
    try {
      ({ signup: updated } = await sendWelcomeEmailForSignup(updated));
    } catch (e) {
      console.error(`[miiaProvisioning] welcome email step crashed for signup ${updated.id}:`, e.stack || e.message);
      await logEmailFailure({
        context: "welcome-email-crashed",
        to: updated.email,
...
// The welcome email's link authenticates on click (7 days - longer than the
// 30-minute return-visit magic link, since this is the one link a customer
// might not open same-day) and lands on the intake chat if they haven't
// finished it yet, or straight on their dashboard if they have -
// app/[tenant]/page.jsx branches on deployedAt either way.
//
// `force: true` (the admin "Resend welcome email" button) sends again even
// if welcomeEmailSentAt is already set - a fresh, still-valid link, useful
// when a customer says they never got the first one.
export async function sendWelcomeEmailForSignup(signup, { force = false } = {}) {
  if (!signup.email || !signup.tenantSlug) return signup;
  if (signup.welcomeEmailSentAt && !force) return signup;

  const welcomeLinkToken = await createMagicLink({ tenantSlug: signup.tenantSlug, ttlMs: WELCOME_LINK_TTL_MS });
  const intakeLink = `${SITE_URL}/api/miia/auth/verify?token=${encodeURIComponent(welcomeLinkToken)}`;
  const firstName = (signup.contactName || "").trim().split(/\s+/)[0] || null;
  ...
  if (result.ok) {
    return updateSignup(signup.id, { welcomeEmailSentAt: new Date().toISOString() });
  }
  return signup;
}
