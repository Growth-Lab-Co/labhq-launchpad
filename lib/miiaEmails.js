...
// Sign-in link for the customer dashboard - both the welcome email's first
// link and a requested return-visit link use this same template. Single-use,
// 30 minutes (see lib/miiaCustomerAuth.js), so the copy says so plainly
// rather than implying it's a permanent bookmark.
export function buildMagicLinkEmail({ firstName, link }) {
  const name = firstName || "there";
  const subject = "Your Miia dashboard link";

  const text = `Hi ${name},

Here's your link to Miia: ${link}

It signs you straight in and works once, for the next 30 minutes. If it's expired by the time you click it, just ask for a new one from the dashboard.`;

  const html = wrapPlain(`
    <p style="margin:0 0 16px;">Hi ${name},</p>
    <p style="margin:0 0 16px;">Here's your link to Miia: <a href="${link}" style="color:#A070F8;">Open your dashboard</a>.</p>
    <p style="margin:0;">It signs you straight in and works once, for the next 30 minutes. If it's expired by the time you click it, just ask for a new one from the dashboard.</p>
  `);
