...
import { ghlCredsFor, tenantProductType } from "./tenants.js";
...
// Found 2026-07-31: a product:"miia" tenant's website chat is the in-house
// widget (job 1, 2026-07-29 build - public/widget.js + widgetKey), not GHL's
// webchat at all - it has no locationId, no botSettings, none of the GHL
// signal channelStatus() above reads. Every such tenant showed "0 of 4
// channels live" regardless of the widget actually being deployed and
// working, because this always fell through to "not_started". The honest
// signal for THIS channel, on THIS architecture, is simply "does a
// deployment exist" - that's the same thing that makes the widget itself
// reachable at all (see lib/tenants.js ensureWidgetKey).
function miiaWebchatStatus(deployment) {
  return deployment ? "live" : "not_started";
}
...
export async function getChannelsData(tenant, deployment, signup) {
  const isMiia = tenantProductType(tenant) === "miia";
  const botSettings = deployment ? await getBotSettings(tenant.slug, deployment.locationId) : null;
  const hasVoice = (signup?.plan || tenant.plan) === "complete";
  // Chat is "one channel of your choice" (job 3, 2026-07-29) - website
  // chat (the widget, live in minutes - see job 2a) is that one channel;
  // SMS and Facebook/Instagram are Everywhere-and-up. Every other plan
  // (Everywhere, Voice/complete, or no plan at all - agency-style/legacy
  // tenants) sees every channel, unchanged from before this restructure.
  const isChatOnly = tenant.plan === "chat";
  ...
  const leadsie = socialLive || isChatOnly ? null : await getLeadsieConnection(tenant.slug).catch(() => null);
  ...
  const channels = [
    {
      id: "webchat",
      label: "Website chat",
      status: isMiia ? miiaWebchatStatus(deployment) : botSettings ? channelStatus(botSettings, "webchat") : "not_started",
    },
    isChatOnly
      ? { id: "sms", label: "SMS", status: "upgrade", hint: "Available on Everywhere" }
      : { id: "sms", label: "SMS", status: botSettings ? channelStatus(botSettings, "sms") : "not_started" },
    isChatOnly
      ? { id: "social", label: "Facebook and Instagram", status: "upgrade", hint: "Available on Everywhere" }
      : {
          id: "social",
  ...
  // hint instead - Voice is demo/invitation-only now (job 3), not a
  // self-serve upgrade button, hence "by invitation" rather than "Available
  // on ...".
  const phone = hasVoice
    ? { id: "phone", label: "Phone", status: "not_started" }
    : { id: "phone", label: "Phone", status: "upgrade", hint: "Voice is by invitation" };
