...
import { getAgencyConnection, getLocationAppToken } from "./ghlOAuth";
...
// prefer a location-app token for this specific location, otherwise fall
// back to the legacy agency PI token (pushAllCustomValues already knows how
// to mint a location token from that on a scope 401). If neither is
// available, logs [LOCATION-AUTH-NEEDED] and returns a null token - callers
// must not fail the deploy on this, just surface it.
export async function resolveLocationDataAuth({ tenantSlug, locationId, legacyCreds }) {
  const locationToken = await getLocationAppToken({ tenant: tenantSlug, locationId });
  if (locationToken) return { token: locationToken, source: "location-app" };
  if (legacyCreds?.configured) return { token: legacyCreds.token, source: "legacy" };
