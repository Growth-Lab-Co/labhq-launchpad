...
function linksStore() {
  return blobStore({ name: LINKS_STORE, consistency: "strong" });
}
function byTenantStore() {
  return blobStore({ name: BY_TENANT_STORE, consistency: "strong" });
}
...
export async function deleteClaimLinksForTenant(slug) {
  const pointer = await byTenantStore().get(slug, { type: "json" });
  if (pointer?.token) await linksStore().delete(pointer.token);
  await byTenantStore().delete(slug);
}
