...
import { createTenant, deleteTenant, getTenant, slugAvailable, validateSlug } from "./tenants.js";
import { registerTenantDomainAlias, netlifyDomainsConfigured } from "./netlifyDomains.js";
...
function accountsStore() {
  return blobStore({ name: ACCOUNTS_STORE, consistency: "strong" });
}

function emailIndexStore() {
  return blobStore({ name: EMAIL_INDEX_STORE, consistency: "strong" });
}
...
// Binds a NEW portal account to an EXISTING tenant slug (the tenant record
// already exists - this is the claim-link path, see lib/claimLinks.js).
// Unlike createAccount, this never creates a tenant record itself; the
// caller (claim redemption route) is responsible for having verified the
// tenant exists first.
export async function createAccountForExistingTenant({ agencyName, tenantSlug, contactEmail, password, acceptedTermsAt }) {
  const email = normaliseEmail(contactEmail);
  ...
  if (!validateEmail(email)) throw new Error("Enter a valid contact email");
  if (!validatePassword(password)) throw new Error("Password must be at least 8 characters");
  if (!acceptedTermsAt) throw new Error("You must agree to the licence terms");

  const tenant = await getTenant(cleanSlug);
  if (!tenant) throw new Error("Unknown tenant");

  const accountId = randomUUID();
  const { modified: emailClaimed } = await setJSONAtomic(emailIndexStore(), email, { accountId }, { onlyIfNew: true });
  if (!emailClaimed) throw new Error("An account with that email already exists");
...
export async function maybeCompleteOnboarding(id) {
  ...
  if (allDone && !o.completedAt) {
    const completed = await updateOnboarding(id, { completedAt: new Date().toISOString() });
    return registerTenantDomain(completed);
  }
  return account;
}

// Best-effort {slug}.labhq.co registration with Netlify, called the moment
// onboarding completes (both the normal signup wizard and the claim-link
// flow route through maybeCompleteOnboarding above, so this one call site
// covers both). Never blocks or fails onboarding completion itself - the
// tenant already works at labhq.co/{slug} regardless, so a failure here
// just leaves domainAlias.status = "failed" for /admin to show and retry
// (see app/api/admin/tenants/[slug]/retry-domain-alias/route.js, which
// calls this same function).
export async function registerTenantDomain(account) {
  if (!netlifyDomainsConfigured()) return account;
  const lastAttempt = new Date().toISOString();
  try {
    await registerTenantDomainAlias(account.slug);
    return updateAccount(account.id, { domainAlias: { status: "ok", lastAttempt, error: null } });
  } catch (e) {
    console.error(`[DOMAIN-ALIAS-FAIL] tenant=${account.slug} status=${e.status ?? "-"}`, e.body ?? e.message);
    return updateAccount(account.id, { domainAlias: { status: "failed", lastAttempt, error: e.message } });
  }
}
...
// Deletes the account record and its email-index entry. Caller (tenant
// removal) is responsible for the tenant record itself and everything else
// tenant-scoped across other stores - this only owns what lib/accounts.js
// owns.
export async function deleteAccount(id) {
  const account = await getAccountById(id);
  if (!account) return;
  await emailIndexStore().delete(normaliseEmail(account.contactEmail));
