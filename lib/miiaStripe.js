...
// Reverse of resolvePriceId - given a Stripe price id actually on a
// subscription, which of our plan ids does it correspond to. Checks every
// MIIA_STRIPE_PRICE_<PLAN>_<MONTHLY|YEARLY|FOUNDING> env var, not just the
// currently-active pricing mode, since a subscription can be sitting on a
// founding price long after MIIA_FOUNDING_MODE has flipped off. Used by the
// subscription.updated webhook backstop to keep tenant/signup.plan in sync
// with whatever's actually on the subscription in Stripe - see
// app/api/miia/webhook/route.js.
export function planForPriceId(priceId) {
  if (!priceId) return null;
  for (const plan of Object.keys(PLAN_ENV_KEY)) {
    const key = PLAN_ENV_KEY[plan];
    for (const suffix of ["MONTHLY", "YEARLY", "FOUNDING"]) {
      if (process.env[`MIIA_STRIPE_PRICE_${key}_${suffix}`] === priceId) return plan;
    }
  }
  return null;
}
...
export async function createMiiaCheckoutSession({
  plan,
  billingPeriod,
  vertical,
  utmSource,
  ...
  // Job (2026-07-30, go-live): White Glove pulled from the live offer -
  // WHITEGLOVE_ENABLED defaults off so it can come back later behind this
  // same flag, without touching this code again. The Stripe price itself
  // is untouched in the catalogue.
  const whiteGlovePriceId =
    process.env.WHITEGLOVE_ENABLED === "true" ? process.env.MIIA_STRIPE_PRICE_WHITEGLOVE_ONCE : null;
  ...
  // HEALTH_VERTICAL_SLUGS. Only set when present so existing sessions/tests
  // aren't affected.
  if (vertical) params["metadata[vertical]"] = vertical;

  // First-touch UTM attribution (see lib/utm.js) - only set when present,
  // same reasoning as vertical above.
...
// Customer dashboard's Billing page "Manage billing" button - Stripe's own
// hosted portal for payment method, invoices, and plan changes. `returnUrl`
// brings them back to the dashboard when they're done.
export async function createMiiaPortalSession({ customerId, returnUrl }) {
  if (!miiaStripeConfigured()) throw new Error("Miia Stripe isn't configured yet");
  return stripeFetch("/billing_portal/sessions", { customer: customerId, return_url: returnUrl });
}

// Self-serve "Upgrade" button on the dashboard (job 4, 2026-07-31) - reads
// the existing subscription's first item so it can be switched to a
// different price. proration_behavior: "create_prorations" is Stripe's
// standard default: the new price takes effect on the subscription
// immediately (which is what unlocks the new channels - see
// app/api/miia/dashboard/upgrade-plan/route.js), while the prorated dollar
// difference is credited/charged on the next invoice rather than an
// immediate separate charge - the normal, well-understood self-serve SaaS
// upgrade pattern.
export async function updateMiiaSubscriptionPrice(subscriptionId, newPriceId) {
  if (!miiaStripeConfigured()) throw new Error("Miia Stripe isn't configured yet");
...
// the customer keeps what they already paid for instead of losing Miia
// mid-cycle. Stripe's portal (above) can also do this; this is the direct
// in-dashboard "Cancel plan" button.
export async function cancelMiiaSubscription(subscriptionId) {
  if (!miiaStripeConfigured()) throw new Error("Miia Stripe isn't configured yet");
