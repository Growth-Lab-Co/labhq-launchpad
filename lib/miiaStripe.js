...
// add-on line item (adjustable_quantity 0-1, defaulting to 0/unticked) -
// Stripe Checkout has no native "optional checkbox" primitive, but a
// quantity-adjustable one-time line item defaulting to zero is the
// well-established, stable pattern for this.
export async function createMiiaCheckoutSession({ plan, billingPeriod, successUrl, cancelUrl }) {
  ...
  if (whiteGlovePriceId) {
    params["line_items[1][price]"] = whiteGlovePriceId;
    params["line_items[1][quantity]"] = "0";
    params["line_items[1][adjustable_quantity][enabled]"] = "true";
    params["line_items[1][adjustable_quantity][minimum]"] = "0";
    params["line_items[1][adjustable_quantity][maximum]"] = "1";
  }
