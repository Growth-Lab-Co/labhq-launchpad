...
export async function recordDeployment({
  ...
  const synced = Boolean(customValuesSynced);
  const record = {
    id: makeId(),
    tenant,
    ...
    locationSyncedAt: synced ? now : null,
    // "pending" only for the Miia Chat tier's instant-deploy path (job 2a) -
    // GHL sub-account creation hasn't run yet, running quietly in the
    // background (see netlify/functions/ghl-provision-background.mjs).
    // Every other path is real GHL data already, hence "ready" by default.
...
export async function deleteDeploymentsForTenant(tenant) {
  const records = await listDeployments(tenant);
...
// Scoped the same way as updateDeploymentStatus - a caller authenticated for
// one tenant can't delete another's record by guessing an id.
export async function deleteDeployment(id, tenant) {
  const record = await getDeployment(id, tenant);
...
// Lets the customer dashboard (or an admin backfill) edit specific
// customValues fields post-deploy - currently just booking_link - without
// touching the rest of the record. lib/bot.js reads customValues fresh on
// every reply, so an edit here takes effect immediately, no redeploy needed.
export async function patchDeploymentCustomValues(id, tenant, patch) {
  const s = store();
  const record = await s.get(id, { type: "json" });
  if (!record || record.tenant !== tenant) return null;
  record.customValues = { ...(record.customValues || {}), ...patch };
  record.updatedAt = new Date().toISOString();
  await s.setJSON(id, record);
  return record;
}
