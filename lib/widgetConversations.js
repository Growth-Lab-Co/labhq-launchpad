...
// `actions` (job 1, 2026-07-31): the fixed-destination suggested-action
// buttons a widget reply can carry - see lib/widgetActions.js. Omitted
// entirely when empty/absent rather than stored as [], so older records
// and non-widget callers are unaffected.
export async function resolvePendingWidgetReply(id, { body, status, actions }) {
  const record = await getWidgetConversation(id);
  if (!record) return null;
  ...
  for (let i = messages.length - 1; i >= 0; i--) {
    if (messages[i].direction === "outbound" && messages[i].status === "pending") {
      ...
      if (actions?.length) messages[i].actions = actions;
      break;
    }
  }
